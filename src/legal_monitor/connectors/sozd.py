...
def _extract_register_date(soup, html: str) -> date | None:
    """Извлечь дату регистрации только из контекста, где она действительно указана.

    Раньше был третий, самый крайний вариант после двух целевых
    попыток: взять первую дату-подобную подстроку в первых 50000
    символах СЫРОГО HTML без какой-либо проверки контекста. Это могло
    легко поймать чужую дату из совсем другого места страницы (номер
    связанного законопроекта, дату из истории рассмотрения, служебную
    разметку/мета-тег) и молча выдать её за дату регистрации — что
    хуже, чем отсутствие даты (тот случай уже корректно исключается и
    считывается в fetch()). Если ни один из двух целевых способов ниже не
    сработал, лучше честно вернуть None, чем угадать.
    """
